 ...
 * @param {string} options.render_instructions para indicar onde vai renderizar as instrucoes. both = default render nos 2 lugares | payer: so no pagador, parte de cima, que fica com quem paga | bank: Compensação, parte de baixo que fica com o banco
 */
var Boleto = function (options) {
  ...
  if (!this['render_instructions']) {
    this['render_instructions'] = 'both'
  }
...
Boleto.prototype.renderHTML = function (callback) {
  ...
  if (Boleto.barcodeRenderEngine == 'bmp') {
    const barcodeDataBuffer = barcode.bmpLineForBarcodeData(self['barcode_data'])
    renderOptions['barcode_data'] = barcodeDataBuffer.toString('base64')
    renderOptions['barcode_data_buffer'] = barcodeDataBuffer
